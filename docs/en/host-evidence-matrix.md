...
|---|---|---|---|
| **Containment gate** (`Test-Containment.ps1`) | **PASS** — verified on PowerShell 7 and PowerShell 7 | UNVERIFIED | UNVERIFIED — hosted CI proof was retired; needs a current local Linux run |
...
| **Cross-platform install path** (`install.sh` / `install.ps1`) | UNVERIFIED — `install.ps1` is the Windows entry point; dry-run works, a full run is not yet measured | UNVERIFIED — intended path, not yet measured | UNVERIFIED — intended path, not yet measured after hosted CI retirement |
...
- The gates are authored to parse under **PowerShell 7** and were verified on
  the local Windows host used for this evidence.
