...
- fan-out은 Codex 자신의 worker GOAL 세션이 한다. 외부 에이전트/peer/MoA/다른
  CLI 브리지를 부르지 않는다.
- worker GOAL이 model capacity, service unavailable, context window, missing
  closeout, or partial output으로 멈추면 Done이 아니다. 레인을
  `MODEL_CAPACITY_RETRY`, `CONTEXT_ROLLOVER_RETRY`, `PARTIAL_RETRY_REQUIRED`,
  `FAILED`, 또는 `BLOCKED`로 분류하고 `worker_recovery_inventory`에 기록한다.
  reversible 작업만 compact evidence로 fresh GOAL/fallback route에서 재시도한다.
