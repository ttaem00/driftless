...
description: >
  ...
  Driftless / 이 저장소를 내 Claude, Codex, 또는 Hermes에 적용 / install this repo for
  ...
  Code, OpenAI Codex, or the bounded Hermes Aemeth environment, or asks the agent
  to onboard them to Driftless.
...
# Apply Driftless (이 레포를 내 Claude/Codex/Hermes에 적용)

이 스킬은 사용자가 "이 레포 적용해줘" 한 문장을 말했을 때 **에이전트가 스스로 따르는
절차**다. 사용자는 코드를 쓰지 않는다. 에이전트가 감지·설치·검증·보고를 한다.

## 절대 규칙 (load-bearing)
- **호스트 전역 설정을 절대 건드리지 않는다.** 격리 홈은 항상 이 레포 안(`.runtime/`)에 만든다. 사용자의 평소 `~/.claude`·`~/.codex`·Hermes 홈은 읽지도 쓰지도 않는다.
...
## 절차
1. **도구 감지.** 지금 도는 게 Claude Code, Codex, Hermes인지 판단한다. Hermes에는 완전한 세 번째 프로필 대신 Aemeth 어댑터만 설치된다고 평이하게 알린다.
2. **드라이런 먼저(부작용 0).** 무엇을 할지 보여준다:
   - macOS/Linux: `sh ./install.sh --dry-run --all` (또는 `--claude`/`--codex`/`--hermes`/`--both`)
   - Windows: `pwsh.exe -ExecutionPolicy Bypass -File .\install.ps1 -DryRun -Tool all`
...
4. **검증(증거 필수, 추측 금지).** 설치 후 실제로 확인한다:
   - 선택한 격리 홈이 `.runtime/claude-home`, `.runtime/codex-home`, `.runtime/hermes-home`에 생겼는가
   - 호스트 전역 홈이 안 바뀌었는가
