...
English enforcement anchor: promote the recurring lesson to the smallest
public-safe surface that can change behavior, then prove it with validation
evidence. Do not stop at a note when a script, skill, hook, prompt, or doc
contract is the root-cause fix.
