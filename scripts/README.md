...
| `Test-CrossAgentWorkArbitrationContract.ps1` | Cross-agent arbitration keeps exact duplicate-work conflicts separate from advisory similar-work discovery. | The arbitration doc lets history act as a default blocker, lets semantic search authorize mutation, or omits the sanitized `purpose_summary` / `intent_tags` boundary. |
