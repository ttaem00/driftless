...
$gates = @(
  ...
  @{ name = 'Cross-agent arbitration contract'; script = 'scripts\Test-CrossAgentWorkArbitrationContract.ps1'; args = @('-Root', $script:RepoRoot); next = 'Restore the public-safe distinction between duplicate-work arbitration and advisory similar-work discovery.' },
  ...
  @{ name = 'Model tier routing contract'; script = 'scripts\Test-ModelTierRoutingContract.ps1'; args = @('-Root', $script:RepoRoot); next = 'Restore provider-detachable role routing, issuance provenance, and fixture coverage before PR_READY.' },
  ...
  @{ name = 'Manager closeout skill suite'; script = 'scripts\Test-ManagerCloseoutSkillSuite.ps1'; args = @('-Root', $script:RepoRoot); next = 'Restore the bounded sprint, twelve-category manager audit, nested evidence safety, and skill evolution fixtures.' },
  ...
  @{ name = 'Worker failure and writer handoff recovery'; script = 'scripts\Test-WorkerFailureRecoveryContract.ps1'; args = @('-Root', $script:RepoRoot); next = 'Restore worker retry states and make-before-break successor-start evidence before releasing an active writer.' },
  ...
  @{ name = 'Detachable Orphanless control module'; script = 'scripts\Test-OrphanlessControlModule.ps1'; args = @('-Root', $script:RepoRoot); next = 'Restore identity materialization, proof lease, deterministic receipt, fission, and parent adoption invariants.' },
  @{ name = 'Orphanless control release integrity'; script = 'scripts\Sync-OrphanlessControlReleaseManifest.ps1'; args = @('-Root', $script:RepoRoot, '-SelfTest'); next = 'Review the public package change, regenerate the release manifest with -Write, and reject stale, missing, or unlisted files.' },
